...
@section('title', "LWP - Edit {{ $product->title }}")
@section('content')
    ...
    <div class="grid sm:grid-cols-6 ">
        ...
        <div class="sm:col-span-5 md:col-span-3 lg:col-span-4 xl:col-span-5 w-full pe-14 pt-5">
            <h2 class="text-center text-lg">Edit {{ $product->title }}</h2>
            <form method="POST" action="{{ route('products.update', ['id' => $product]) }}" class="px-20 flex flex-col ">
                @method('PUT')
                @csrf
                <div class="mb-4">
                    <label for="img"
                ...
                <div class="mb-4">
                    ...
                    <textarea placeholder="Content description of article..." name="description" value="{{ $product->description }}"
                        class="w-full h-32 block shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-blue-300 focus:ring-blue-300">{{ $product->description }}</textarea>
                </div>
                <div class="mb-4">
                    <label for="article"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Article</label>
                    <textarea placeholder="Content of post..." name="article" value="{{ $product->article }}"
                        class="w-full h-52 block shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-blue-300 focus:ring-blue-300">{{ $product->article }}</textarea>
                </div>
                <div class="mb-4">
                    <label for="author"
